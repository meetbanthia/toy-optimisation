...
// =============================================================================
// 3DGRT NAVIGATION INDEX  (added when extracting reference copy from
// vk_gaussian_splatting; the file is the original 3317-line one with code for
// 3DGS raster, 3DGUT, and 3DGRT all interleaved — the line numbers below
// reference the ORIGINAL file, so in this annotated copy add ~75 lines, or
// just grep for the symbol names listed.)
//
//   Setup / teardown
//     L1121-1122  initRtDescriptorSet() + initRtPipeline()  — called during init
//     L2467-2489  destroy of m_computePipelineParticleAs, m_rtPipeline,
//                 m_rtPipelineLayout, m_rtDescriptor*, m_rtSBTBuffer
//
//   Shader compilation (slang -> SPIR-V modules)
//     L1767-1772  compile rgen / rmiss / rmiss_shadow / rchit / rahit / rint
//                 from threedgrt_raytrace.*.slang
//     L1776       compile particle_as_build.comp.slang  (compute that builds
//                 the per-particle AABBs feeding BLAS)
//
//   Particle AS compute pipeline (builds per-splat geometry for BLAS)
//     L1899-1909  m_particleAsPipelineLayout creation (push constants:
//                 shaderio::ParticleAsBuildPushConstants)
//     L2011-2025  m_computePipelineParticleAs (VkComputePipeline)
//     L2048       hand pipeline + layout + descriptor set to
//                 SplatSetManagerVk::setParticleAsComputeState — the manager
//                 then dispatches it whenever a BLAS rebuild is requested
//
//   RT descriptor set
//     L2625-2782  initRtDescriptorSet() — bindings: OUTIMAGE, AUX1, OUTDEPTH,
//                 DLSS_OUT_IMAGES (set RTX_BINDING_*; storage images for the
//                 rgen shader to write into). Pool + set alloc + writes.
//
//   RT pipeline + SBT
//     L2785-2956  initRtPipeline() — stages (rgen/miss/miss2/chit/ahit/rint),
//                 shader groups (raygen, 2x miss, hit group for splats,
//                 hit group for meshes), pipeline layout combining
//                 m_descriptorSetLayout + m_rtDescriptorSetLayout,
//                 vkCreateRayTracingPipelinesKHR, SBT buffer fill.
//
//   Per-frame dispatch
//     L2959+      raytrace(cmd, meshDepthOnly) — binds m_rtPipeline + descriptor
//                 sets, sets push constants, vkCmdTraceRaysKHR.
//     Call sites: L463 (primary), L562 (mesh-depth pre-pass for hybrid 3DGUT),
//                 L920 (secondary rays for meshes/lights).
//
//   Request flags driving BLAS/TLAS rebuilds
//     L1070       sets SplatSetManagerVk::Request::eRebuildBLAS when splats
//                 mutate
//     L1076-1085  RTX request mask: UpdateTransformsOnly | eRebuildTLAS |
//                 eRebuildBLAS (handled deferred, no device wait)
//
//   Properties query
//     L136-137    VkPhysicalDeviceRayTracingPipelinePropertiesKHR (used for
//                 SBT handle size + alignment)
//
//   Supporting files in this folder
//     particle_acceleration_structures_gpu.{cpp,hpp}  — GPU-driven BLAS/TLAS
//         builder used by 3DGRT for the gaussian particle BVH
//     acceleration_structures_lb.{cpp,hpp}            — load-balanced AS build
//         helpers (used for mesh BLAS in the hybrid path)
//     splat_set_manager_vk.{cpp,h}                    — owns particle AS state,
//         dispatches the particle_as_build compute pipeline, requests rebuilds
//     splat_set_vk.{cpp,h}, splat_set.h               — per-splat-set GPU
//         buffers + CPU-side data
//
// Note: this file references many helpers from nvpro_core2/nvvk and the rest
// of the original src/ tree (asset_manager_vk, dlss_*, parameters, etc.) that
// were NOT copied here — this is a read-only study reference, not a buildable
// extract.
// =============================================================================
